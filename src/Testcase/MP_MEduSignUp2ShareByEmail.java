...
public class MP_MEduSignUp2ShareByEmail {
	public JSONArray actualEvents;
	public JSONArray actualEvents_Mem;
	public O_Account acc = new O_Account(Utils.generateRandomEmail("eduM"));
	public O_Onboarding onb = new O_Onboarding("School/University", "Student", "Resume");
	...
	public String sharedEmail = "testshare@venngage.com";
	...
	public void beforeClass() throws InterruptedException {
		...
		Constants.driver.navigate().to(Constants.EduM_SignUp_Url);
		acc.Register();
		...
		F_Editor.shareDesignByEmail(sharedEmail);
		
		//Cancel account
		Constants.driver.navigate().to(Constants.Subscriptions_Url);
		F_MyAccount.cancelSubscription("9", 2, "");
	}
	...
	public void verifyEventsGenerated() throws IOException, JSONException, InterruptedException {
		List<String> exp_Events = new LinkedList<String>(Arrays.asList("Visit Homepage", "Register",
				...
				, "Click on Content Gate onboarding", "Click on Cancel Subscription", "Cancelled Education"));
		// Call mixpanel api
	...
	public void e_Share() throws JSONException {
		...
		//assertTrue(F_Mixpanel_API.verifyEventProperties(event, "Infographic", createdTemplateId));
	...
	public void intercomEvents() throws JSONException
	{
		List<String> expectedEvents = Arrays.asList("upgrade education admin" ,"create infographic"
													, "share with user", "cancelled subscription");
		...
		//Verify share with user event
		JSONObject e_Share = F_Intercom_API.getEventByName(actualEvents, "share with user");
		assertTrue(F_Intercom_API.verifyEventProperty(e_Share, "infographic_id", infographId));
		assertTrue(F_Intercom_API.verifyEventProperty(e_Share, "email", sharedEmail));
	}
